...
class TestDeleteTask:
    ...
    def test_successful_delete(
        ...
        url = reverse("tasks:delete-task", args=[project.id, task.id])
    ...
    def test_delete_nonexistent_task(
        ...
        url = reverse("tasks:delete-task", args=[project.id, 999])
        api_client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
    ...
    def test_delete_task_wrong_project(
        self, api_client: APIClient, token: str, task: Task
    ):
        """Test deleting a task with wrong project ID."""
        wrong_project_id = 999
        url = reverse("tasks:delete-task", args=[wrong_project_id, task.id])
        api_client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
