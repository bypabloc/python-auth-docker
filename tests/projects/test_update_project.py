...
class TestUpdateProject:
    ...
    def test_successful_update(
        ...
        assert response.data["data"]["name"] == new_data["name"]
        assert response.data["data"]["description"] == new_data["description"]
        assert response.data["data"]["is_active"] == new_data["is_active"]
        assert response.data["message"] == "Project updated successfully"

        # Verify updates in database
        project.refresh_from_db()
        assert project.name == new_data["name"]
        assert project.description == new_data["description"]
        assert project.is_active == new_data["is_active"]
    ...
    def test_update_as_member(
        ...
        """Test project update attempt by regular member."""
    ...
    def test_update_nonexistent_project(self, api_client: APIClient, token: str):
        """Test updating a nonexistent project."""
        url = reverse("projects:update-project", args=[999])
        api_client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
    ...
    def test_update_with_invalid_data(
        ...
        response = api_client.put(url, {"name": "", "description": "test description"})

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "name" in str(response.data["errors"])

    def test_update_without_auth(self, api_client: APIClient, project: Project):
