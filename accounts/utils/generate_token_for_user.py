...
def generate_token_for_user(user, request, is_temporary=False):
    ...
    expires_at = timezone.now() + (timedelta(minutes=10) if is_temporary else timedelta(days=7))
    ...
    user_token = UserToken.objects.create(
        ...
        device_browser=f"{parsed_ua['user_agent']['family']} {parsed_ua['user_agent']['major']}",
